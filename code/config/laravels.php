...
return [
    ...
    'timer'                    => [
        ...
        'jobs'          => [
            ...
            \App\Jobs\TestSwooleCronJob::class,
        ],
        'max_wait_time' => 5,//Reload时最大等待时间
    ],
    'events'                   => [
        \App\Events\TestTaskEvent::class => [
            \App\Listeners\TestTaskEventListener::class,
        ]
    ],
    'swoole_tables'            => [
        'ws' => [ // 表名，会加上 Table 后缀，比如这里是 wsTable
            ...
            'column' => [ // 表字段，字段名为 value
                ...
                ['name' => 'name', 'type' => \Swoole\Table::TYPE_STRING, 'size' => 8],
                ['name' => 'score', 'type' => \Swoole\Table::TYPE_FLOAT],
            ],
        ],
    ],
    'register_providers'       => [],
