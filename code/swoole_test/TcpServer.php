...
class TcpServer{
    ...
    public function run(){
        ...
        $process = new Process(function(){
            // 获取当前进程的id作为主进程 ID
            $this->mpid = posix_getpid();//此函数表示获取当前进程的id
            echo time() . " 主进程 pid {$this->mpid}\n";

            // 创建 TCP 服务器并获取套接字
            $this->socket = stream_socket_server("tcp://192.168.10.10:5300", $errno, $errstr);
            if (!$this->socket) {
                exit("Server start error: $errstr --- $errno");//启动失败
            }
    ...
    public function checkMpid(&$worker)
    {
        ...
        if (!Process::kill($this->mpid, 0)) {
            $worker->exit();
            // 这句提示，实际是看不到的，需要写到日志中，因为是在子进程中输出，不会在控制台中看到
